...
class TrainDataset(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=True, stride=8):
        ...
        with open(f'{data_root}/split_txt/train_list.txt', 'r') as fin:
            ...
            bgr_list = [line.replace('.mat','_RGB_raw.png') for line in hyper_list]
            nir_list = [line.replace('.mat','_NIR940.png') for line in hyper_list]
...
class ValidDataset(Dataset):
    def __init__(self, data_root, bgr2rgb=True):
        ...
        with open(f'{data_root}/split_txt/valid_test.txt', 'r') as fin:
            hyper_list = [line.replace('\n', '.mat') for line in fin]
            bgr_list = [line.replace('.mat','_RGB_AWB.png') for line in hyper_list]
            nir_list = [line.replace('.mat','_NIR940.png') for line in hyper_list]
        hyper_list.sort()
        bgr_list.sort()
        nir_list.sort()
