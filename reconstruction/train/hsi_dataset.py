...
class TrainDataset(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=True, stride=8):
        ...
        bgr_data_path = f'{data_root}/Train_RGBN/'

        with open(f'{data_root}/split_txt/train_list.txt', 'r') as fin:
            hyper_list = [line.replace('\n','.mat') for line in fin]
            bgr_list = [line.replace('.mat','_RGB.png') for line in hyper_list]
            nir_list = [line.replace('.mat','_NIR.png') for line in hyper_list]
            nir_list1 = [line.replace('.mat','_NIR.png') for line in hyper_list]
        hyper_list.sort()
        bgr_list.sort()
