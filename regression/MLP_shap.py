...
# logging
log_dir = os.path.join(os.getcwd(), 'shap.log')
...
#model = load_model('./Models/FFNN_hyper_OldDataset_204.pth')
model = MLPRegressorPyTorch((200, 100, 16), random_state=42, batch_size=len(X_train), learning_rate_init=0.001, max_iter=200, verbose=True)

shap_model = model.model
shap_model.fit(X_train, y_train, X_test, y_test)
...
logger.info(f"X_train_ shape before sampling: {X_train_.shape}")

logger.info("Shap explainer begins...")

# X_train_ = shap.sample(X_train_, 100000)
# X_test_ = shap.sample(X_test_, 30000)

logger.info(f"X_train_ shape after sampling: {X_train_.shape}")

# Create a SHAP explainer
explainer = shap.DeepExplainer(shap_model, X_train_)
# Compute SHAP values for the test set
shap_values = explainer.shap_values(X_test_)

logger.info("Shap explainer ends...")

# Create and save the SHAP summary plot
plt.figure()
shap.summary_plot(shap_values, X_test_, feature_names=BANDS_WAVELENGTHS, max_display=204, show=False)
plt.savefig('MLP_shap_builtin_summary_plot_AllSample.png')
...
plt.savefig('MLP_shap_summary_plot_AllSample.png')


save_model(explainer, './Models/SHAP_Explainer_MLP_OldDataset_204_AllSample.pkl')
explainer_shap = explainer(X_test_)
save_model(explainer_shap, './Models/Explainer_shap_MLP_OldDataset_204_AllSample.pkl')
np.save('./Models/shap_values_MLP_OldDataset_204_AllSample.npy', shap_values)

logger.info("Reach the end of file...")
