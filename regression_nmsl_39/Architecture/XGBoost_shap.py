...
# logger.info(f"X_train_ shape: {X_train.shape}")
...
# # Save the SHAP explainer and SHAP values
# save_model(explainer, './Models/SHAP_Explainer_xgboost.pkl')

# Load the SHAP explainer and explainer object
# explainer = load_model('./Models/SHAP_Explainer_xgboost.pkl')
explainer_shap = load_model('./Models/ExplainerObject_Shap_xgboost.pkl')
explainer_shap.feature_names = BANDS_WAVELENGTHS

# np.save('shap_values_xgboost.npy', shap_values)
shap_values = np.load('shap_values_xgboost.npy')
# explainer_shap = explainer(X_test)

# save_model(explainer_shap, './Models/ExplainerObject_Shap_xgboost.pkl')

logger.info("Shap explainer ends...")

# # Create and save the SHAP summary plot
# plt.figure()
# shap.summary_plot(shap_values, X_test, feature_names=BANDS_WAVELENGTHS, max_display=204, show=False)
# plt.savefig('XGB_shap_builtin_summary_plot.svg', bbox_inches='tight')
...
plt.figure()
shap.plots.beeswarm(explainer_shap, max_display=204, show=False)
plt.savefig('XGB_shap_beeswarm_plot.svg', bbox_inches='tight')
plt.close()
